#
# $Id: alcdxtconv.py 856 2007-10-31 19:12:27Z trylon $
#
...
class tImage:
    ...
    def save(self,name):
        if not glob.glob(name):
            ...
            #self.fromRGBA()
            #self.toRGBA()
            # Sirius: modified for TGA support
            if (not name.endswith(".png")) and (not name.endswith(".tga")):
                self.applyAlpha()
            im = Image.new("RGBA",(self.w,self.h))
            #self.data.read()
            ...
            if name.endswith(".tga"):
                # tga stores the lines differently from png
                im = im.transpose(Image.FLIP_TOP_BOTTOM)
            im.save(name)
        else:
            print "       (MipMap already extracted)"
